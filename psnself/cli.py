...
def headless_sync():
    ...
    if not config.get("npsso"):
        print("Error: Not authenticated. Run 'psnself' and use the auth screen.")
        sys.exit(1)
...
def headless_sync_friends():
    ...
    online_id = config.get("online_id", "unknown")
    print(f"Syncing friends leaderboard for {online_id}...")

    def on_progress(current, total, name):
        if total > 0:
            pct = int((current / total) * 100)
            print(f"  [{pct:3d}%] ({current}/{total}) {name}", end="\r")
        else:
            print(f"  ({current}) {name}", end="\r")

    result = sync.fetch_friends_leaderboard(config["npsso"], progress_callback=on_progress)
    print()
    print(f"Done: {result['processed']} friends processed, "
          f"{result['private']} private, {result['errors']} errors, "
          f"{result['games_stored']} game entries")
    db.close_conn()
...
def main():
    parser = argparse.ArgumentParser(
        prog="psnself",
        description="PSN Trophy Tracker — web dashboard + CLI sync",
    )
    parser.add_argument(
        "--sync", action="store_true",
        help="Run headless trophy sync (for cron/systemd)",
    )
    parser.add_argument(
        "--sync-friends", action="store_true",
        help="Run headless friends leaderboard sync (for cron/systemd)",
    )
    parser.add_argument(
        "--sync-all", action="store_true",
        help="Run both trophy and friends sync",
    )
    parser.add_argument(
